    ...
    if (booking.paymentModel === 'PAID_IN_FULL') {
        ...
        return (
            <div className='flex items-baseline justify-between gap-4 rounded-[10px] bg-it-green-subtle px-3.5 py-2.5'>
                <span className='text-[13px] leading-[1.6] text-it-green tracking-[-0.012em]'>
                    {dict.payPaidInFull}
                </span>
                <span className='text-right text-[13px] leading-[1.6] font-medium text-it-green tracking-[-0.012em]'>
                    {total}
                </span>
            </div>
        );
...
function MoneyRow({ label, value }: { label: string; value: string }) {
    return (
        <div className='flex items-baseline justify-between gap-4'>
            ...
            <span className='text-right text-[13px] leading-[1.6] font-medium text-it-heading tracking-[-0.012em]'>
                {value}
            </span>
        </div>
    );
...
function Note({ children }: { children: string }) {
    return (
        <p className='mt-1 mb-0 text-[12px] leading-[1.6] text-it-heading/80 tracking-[-0.012em]'>
            {children}
        </p>
    );
}
