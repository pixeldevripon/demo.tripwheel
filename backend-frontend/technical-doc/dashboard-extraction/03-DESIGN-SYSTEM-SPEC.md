...
- **`@theme`** defines tokens and _generates utilities_ from them.
...
@theme {
    ...
    --color-n-200: oklch(0.9 0.007 250);
    --color-n-300: oklch(0.84 0.009 250);
    --color-n-400: oklch(0.7 0.012 250);
    --color-n-450: oklch(
        0.65 0.012 250
    ); /* ADDED 2026-07-17 - light --line-control (3.09:1) */
    --color-n-500: oklch(0.58 0.014 250);
    --color-n-550: oklch(
        0.55 0.014 250
    ); /* ADDED 2026-07-17 - light --content-subtle (4.64:1);
    ...
    --color-n-900: oklch(0.21 0.01 250);
...
:root {
    ...
    --content-subtle: var(
        --color-n-550
    ); /* tertiary - lowest allowed. n-500 measured 4.10:1; see §9 */
    --content-inverse: var(--color-n-0);

    --line: var(
        --color-n-200
    ); /* default border - DECORATIVE, no contrast target */
    --line-strong: var(--color-n-300); /* DECORATIVE, no contrast target */
    --line-subtle: var(--color-n-100);
    --line-control: var(
        --color-n-450
    ); /* 3.09:1 - WCAG 1.4.11. Inputs/checkboxes. See §9 */
    ...
    --warning-solid: oklch(0.7 0.15 75);
    ...
    --chart-6: oklch(0.6 0.11 190);

    --shadow-xs: 0 1px 2px 0 oklch(0.21 0.01 250 / 0.05);
    --shadow-sm:
        0 1px 3px 0 oklch(0.21 0.01 250 / 0.08),
        0 1px 2px -1px oklch(0.21 0.01 250 / 0.06);
    --shadow-md:
        0 4px 8px -2px oklch(0.21 0.01 250 / 0.1),
        0 2px 4px -2px oklch(0.21 0.01 250 / 0.06);
    --shadow-lg:
        0 12px 20px -4px oklch(0.21 0.01 250 / 0.12),
        0 4px 8px -4px oklch(0.21 0.01 250 / 0.08);
...
.dark {
    ...
    --line-control: oklch(
        0.5 0.014 250
    ); /* 3.39:1 - no ramp step clears 3:1 on n-1000 */
    ...
    --shadow-md:
        0 4px 8px -2px oklch(0 0 0 / 0.45), 0 2px 4px -2px oklch(0 0 0 / 0.3);
    --shadow-lg:
        0 12px 20px -4px oklch(0 0 0 / 0.55), 0 4px 8px -4px oklch(0 0 0 / 0.4);
...
#### 5.4.1 A sheet has three slots, and they share one inset

`SheetHeader`, **`SheetBody`** and `SheetFooter`. All three carry the same 32px horizontal
padding (`p-8` / `px-8`), so the title, the content and the actions line up down one edge.

`SheetBody` was added 2026-08-02. Before it, every sheet hand-rolled its own scroll container -
and all eleven of them reached for `px-4` against a `p-8` header, so the content sat 16px
further in than the title above it on every detail screen in the dashboard. The inset is a
property of the sheet, not of whatever happens to be inside it, so it now lives with the other
two slots and cannot drift again.

`SheetBody` also absorbs the `min-h-0 flex-1 overflow-y-auto` that each sheet was repeating.
Only the HORIZONTAL padding is fixed: vertical rhythm stays the caller's, because a detail
sheet divides its own `Section`s (each carrying `py-4`) while a form wants a gap. Pass it in
`className`.

**Rule:** never hand-roll a scrolling `<div>` inside a `SheetContent`. Use `SheetBody`.
...
> which reads as a _secondary_ action. "Delete tour" should not look quieter than "Save".
...
> available _because_ the lint ran first, and it corrected the scale before a single token was
> written.
