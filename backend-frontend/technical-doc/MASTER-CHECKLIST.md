...
- [~] GTM container four-tag fan-out (Conversion Linker · Google Ads · GA4 `purchase` · Meta Pixel) — **GTM-UI configuration, not code**: the full recipe lives in `technical-doc/03-implementation/GTM-CONTAINER-SETUP.md` (the Meta tag MUST pass `eventID = {{dlv - event_id}}` or it double-counts vs CAPI); awaiting the founder's container work. Step-by-step: `03-implementation/SEO-AND-TRACKING-SETUP-RUNBOOK.md`
- [x] GA4 loads from the dashboard `googleAnalyticsId` (2026-08-19) — the field was inert and GA4 secretly depended on a "Google tag" inside the container; `google-tag-manager.tsx` now loads `gtag.js` from it, GA4 and GTM are independently gated, and both IDs are format-validated + uppercase-normalised (`lib/tracking/tag-ids.ts` — `gtm.js?id=` is case-SENSITIVE, so a lowercase paste used to emit a loader that 404'd and silently killed the whole fan-out). Consent-defaults-before-loaders is now pinned by tests (`lib/tracking/google-tags-snippet.ts`). **The container must NOT also carry a GA4 configuration tag** or pageviews double-count. The Google Ads Conversion ID stays in GTM deliberately — firing it from the app too would double-count every booking
