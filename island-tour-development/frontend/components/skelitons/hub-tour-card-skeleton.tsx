 ...
 * `aspect-177/148` → `md:aspect-384/270`, content
 * `gap-1.5 pt-2` → `md:gap-3 md:pt-4`. The save button is absolute, so it does
 * not affect layout height.
 */
export function HubTourCardSkeleton({ className }: { className?: string }) {
    return (
        <div
            ...
            <div className='relative aspect-177/148 w-full shrink-0 rounded-[8px] md:aspect-384/270 md:rounded-[16px]'>
            ...
            {/* Content - same gaps + top padding as the real card. */}
            <div className='flex flex-col gap-1.5 pt-2 md:gap-3 md:pt-4'>
                {/* Rating. */}
