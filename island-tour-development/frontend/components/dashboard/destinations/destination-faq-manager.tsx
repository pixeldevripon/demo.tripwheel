...
import type { DestinationFaq } from '@/types/destination';
...
function FaqCard({ faq, destinationId }: FaqCardProps) {
...
function CreateFaqDialog({ destinationId, onSuccess }: CreateFaqDialogProps) {
...
export function DestinationFaqManager({ destinationId }: DestinationFaqManagerProps) {
  const [localeFilter, setLocaleFilter] = useState<string>('all');
  const { data: faqs, isLoading } = useDestinationFaqs(
    destinationId,
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <CreateFaqDialog destinationId={destinationId} onSuccess={() => {}} />
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {filteredFaqs.map((faq) => (
            <FaqCard key={faq.id} faq={faq} destinationId={destinationId} />
          ))}
        </div>
      )}
    </div>
