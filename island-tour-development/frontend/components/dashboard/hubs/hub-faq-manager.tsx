...
import type { HubFaq } from '@/types/hub';
...
function FaqCard({ faq, hubId }: FaqCardProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [deleteOpen, setDeleteOpen] = useState(false);
  const { mutate: updateFaq, isPending: isUpdating } = useUpdateHubFaq();
  const { mutate: deleteFaq, isPending: isDeleting } = useDeleteHubFaq();
...
function CreateFaqDialog({ hubId, onSuccess }: CreateFaqDialogProps) {
  const [open, setOpen] = useState(false);
  const { mutate: createFaq, isPending } = useCreateHubFaq();
...
interface HubFaqManagerProps {
  hubId: string;
}

export function HubFaqManager({ hubId }: HubFaqManagerProps) {
  const [localeFilter, setLocaleFilter] = useState<string>('all');
  const { data: faqs, isLoading } = useHubFaqs(
    hubId,
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <CreateFaqDialog hubId={hubId} onSuccess={() => {}} />
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {filteredFaqs.map((faq) => (
            <FaqCard key={faq.id} faq={faq} hubId={hubId} />
          ))}
        </div>
      )}
    </div>
