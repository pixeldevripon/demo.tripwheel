    ...
    <TripDetailShell id={id} name={trip.name} isLoading={false} subtitle="Edit trip">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div className="flex items-center gap-3">
          ...
          {trip.isSponsored && <Badge variant="outline">Sponsored</Badge>}
        </div>

        {can('MANAGE_TRIPS') && (
          <div className="flex flex-wrap gap-2">
            {trip.status === 'DRAFT' && (
              <Button size="sm" onClick={handlePublish} disabled={isLifecyclePending}>
            ...
            {trip.status === 'LIVE' && (
              <>
                <Button size="sm" variant="outline" onClick={handlePause} disabled={isLifecyclePending}>
            ...
            {trip.status === 'PAUSED' && (
              <>
                <Button size="sm" onClick={handleUnpause} disabled={isLifecyclePending}>
      ...
      {/* Publish readiness */}
      {trip.status === 'DRAFT' && (
        <Card className={`mb-6 ${allPassed ? 'border-emerald-200' : 'border-amber-200'}`}>
