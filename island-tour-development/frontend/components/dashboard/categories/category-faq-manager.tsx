...
function FaqCard({ faq, categoryId }: FaqCardProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [deleteOpen, setDeleteOpen] = useState(false);
  const { mutate: updateFaq, isPending: isUpdating } = useUpdateCategoryFaq();
  const { mutate: deleteFaq, isPending: isDeleting } = useDeleteCategoryFaq();
            ...
            <form onSubmit={handleSubmit(onSubmitEdit)} className="space-y-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2 mt-5">
                  ...
                  />
                  <Label htmlFor={`isActive-${faq.id}`} className="text-xs font-semibold uppercase cursor-pointer">
                    Active
                  </Label>
                </div>
              </div>
...
function CreateFaqDialog({ categoryId, onSuccess }: CreateFaqDialogProps) {
  const [open, setOpen] = useState(false);
  const { mutate: createFaq, isPending } = useCreateCategoryFaq();
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-2xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Field>
            <Label className="text-xs font-semibold uppercase">Question</Label>
            <Input {...register('question')} placeholder="What is..." aria-invalid={!!errors.question} />
            <FieldError>{errors.question?.message}</FieldError>
          </Field>

          <Field>
            <Label className="text-xs font-semibold uppercase">Answer</Label>
            <Textarea {...register('answer')} placeholder="The answer is..." rows={4} aria-invalid={!!errors.answer} />
            <FieldError>{errors.answer?.message}</FieldError>
          </Field>
...
interface CategoryFaqManagerProps {
  categoryId: string;
}

export function CategoryFaqManager({ categoryId }: CategoryFaqManagerProps) {
  const [localeFilter, setLocaleFilter] = useState<string>('all');
  const { data: faqs, isLoading } = useCategoryFaqs(
    categoryId,
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <CreateFaqDialog categoryId={categoryId} onSuccess={() => {}} />
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {filteredFaqs.map((faq) => (
            <FaqCard key={faq.id} faq={faq} categoryId={categoryId} />
          ))}
        </div>
      )}
    </div>
