...
function LocaleTab({ categoryId, locale, disableNameField }: LocaleTabProps) {
  const { data: translation, isLoading } = useCategoryTranslationByLocale(categoryId, locale);
  const { mutate: upsert, isPending: isUpserting } = useUpsertCategoryTranslation();
  const { mutate: deleteTranslation, isPending: isDeleting } = useDeleteCategoryTranslation();
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <Field>
        ...
        <FieldDescription>Overrides the default H1 heading on the category page.</FieldDescription>
      </Field>
        ...
        <Tabs defaultValue="en">
          ...
          {(['es', 'nl', 'pt', 'fr', 'de', 'zh'] as Locale[]).map((locale) => (
            <TabsContent key={locale} value={locale}>
              <LocaleTab categoryId={categoryId} locale={locale} />
            </TabsContent>
          ))}
        </Tabs>
      </CardContent>
