...
export const inputClass =
    'w-full rounded-[10px] border border-it-border bg-it-white px-3.5 py-2.75 text-[16px] md:text-[15px] text-it-heading placeholder:text-it-ink-placeholder focus:border-transparent focus:outline-2 focus:outline-it-primary tracking-[-0.012em]';
...
export const primaryBtn =
    'flex w-full items-center justify-center gap-2 rounded-full bg-it-primary px-5 py-3.25 text-[15px] font-medium text-it-primary-fg transition-[filter] hover:brightness-95 tracking-[-0.012em]';

export const quietLink =
    'text-[13.5px] font-medium text-it-text-muted transition-colors hover:text-it-primary tracking-[-0.012em]';
...
export function Field({
    ...
    return (
        <div className='mb-4'>
            <label
                htmlFor={htmlFor}
                className='mb-1.5 block text-[13px] font-medium text-it-heading tracking-[-0.012em]'>
                {label}
            </label>
            {children}
        </div>
...
export function FieldError({
    ...
    return (
        <p
            id={id}
            role='alert'
            className='mt-1.5 text-[13px] leading-[1.5] text-red-600 tracking-[-0.012em]'>
            {children}
        </p>
    );
}
...
export function ErrorNote({ children }: { children: React.ReactNode }) {
    return (
        <div
            role='alert'
            className='mb-4 flex items-start gap-2 rounded-[10px] border border-red-200 bg-red-50 px-3.25 py-2.5 text-[13.5px] text-red-700 tracking-[-0.012em]'>
...
export function SuccessBlock({
    ...
    return (
        <div className='py-2 text-center'>
            ...
            <strong className='block text-[17px] text-it-heading tracking-[-0.012em]'>{title}</strong>
            <p className='mt-2 text-[14px] leading-relaxed text-it-text-muted tracking-[-0.012em]'>{body}</p>
            {loginHref && (
                <Link
                    href={loginHref}
                    className='mt-5 inline-flex items-center justify-center rounded-full border border-it-border bg-it-white px-6 py-2.5 text-[14px] font-medium text-it-heading transition-colors hover:border-it-primary hover:text-it-primary tracking-[-0.012em]'>
                    {loginLabel}
                </Link>
            )}
        </div>
