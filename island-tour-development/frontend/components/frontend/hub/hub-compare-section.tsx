    ...
    return (
        <Reveal className='flex flex-col gap-[22px]'>
            ...
            <header className='flex flex-col gap-1'>
                ...
                <p className='m-0 max-w-[530px] it-text text-it-text-muted'>
                    {dict.subtitle}
                </p>
            </header>
...
function CompareTableCard({
    ...
    // the room comes from.
    const cell =
        'flex items-center px-3 lg:px-4 py-[11px] it-text border-it-divider text-it-text-muted';
    ...
    return (
        <div className='@container overflow-hidden rounded-it-lg border border-it-divider bg-it-white shadow-it-sm'>
            ...
            <div className='border-b border-it-peach-border bg-it-primary-subtle px-4 py-2.5 lg:px-4'>
                <span className='text-[14.5px] md:text-[14.5px] font-bold uppercase tracking-[-0.012em] text-it-primary leading-[1.6]'>
                    {title}
                </span>
            </div>
...
function CompareValue({ value }: { value: CompareCell }) {
    ...
    return (
        <span className='flex flex-wrap items-center gap-x-2 gap-y-0.5 it-text text-it-heading'>
            {value.check && (
                <Image
                    src={CHECK_ICON}
                    alt=''
...
function PriceLabel({ boat, from }: { boat: CompareBoat; from: string }) {
    return (
        <span className='flex items-center gap-2 it-text tabular-nums'>
            {boat.priceCheck && (
                <Image
                    src={CHECK_ICON}
                    alt=''
            ...
            <span className='text-it-text-muted tracking-[-0.012em]'>
                {from}{' '}
                <span className='text-[13px] font-medium tracking-[-0.012em] text-it-heading'>
                    {boat.priceDisplay}
                </span>
...
function BookButton({ label, href }: { label: string; href?: string }) {
    const className =
        'inline-flex shrink-0 cursor-pointer items-center justify-center gap-1.5 rounded-it-full bg-it-primary it-text  font-medium text-it-white no-underline transition-colors duration-(--it-duration-xs) hover:bg-it-primary-hover h-[33px] px-2 lg:px-[18px]';
    if (href) {
        return (
            <MotionLink
                href={href}
