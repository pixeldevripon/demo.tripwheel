            ...
            {isEmpty ? (
                <div className='flex flex-col items-start gap-4 py-6 md:py-10'>
                    ...
                    <button
                        type='button'
                        onClick={() => setDate(undefined)}
                        className='cursor-pointer rounded-it-full border border-it-heading bg-transparent px-5 py-2 text-[14px] font-medium leading-[1.6] tracking-[-0.012em] text-it-heading'>
                        {filter.showAllDates}
                    </button>
                </div>
            ) : (
                <div
                    className={`flex flex-col transition-opacity ${grouped ? 'gap-8 md:gap-12' : ''} ${checking ? 'opacity-60' : ''}`}>
                    {grouped
                        ...
                        : filteredGroups[0] && (
                              <Reveal className={GRID}>
                                  {filteredGroups[0].tours.map((tour) => (
                                      <HubTourCard
                                          key={tour.id}
                                          tour={tour}
                                          dict={card}
