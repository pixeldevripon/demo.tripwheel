 ...
 *
 * Covers /dashboard/collections (list) and /dashboard/collections/new (create),
 * plus the edit view sub-tabs.
 *
 *  1. List page (/dashboard/collections)
...
test.describe('Collections — list page (/dashboard/collections)', () => {
  ...
  test('"Add Collection" button is visible and navigates to /new', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/dashboard\/collections\/new/, { timeout: 5_000 });
  });
  ...
  test('edit icon navigates to the collection edit page', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/dashboard\/collections\/col-id-1\/edit/, { timeout: 5_000 });
  });
...
test.describe('Collections — create form MANUAL type (/dashboard/collections/new)', () => {
  ...
  test('happy path: valid MANUAL form submits via POST and navigates to /collections', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/dashboard\/collections$/, { timeout: 8_000 });
  });
});
...
test.describe('Collections — edit view sub-tabs', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto(`/dashboard/collections/${COLLECTION_ID}/edit`);
    await page.waitForSelector('form', { timeout: 15_000 });
  });
  ...
  test('navigating to Translations page renders the translations view', async ({ page }) => {
    ...
    await expect(page).toHaveURL(new RegExp(`/dashboard/collections/${COLLECTION_ID}/translations`), { timeout: 5_000 });
  });
  ...
  test('navigating to Page Content page renders the page-content view', async ({ page }) => {
    ...
    await expect(page).toHaveURL(new RegExp(`/dashboard/collections/${COLLECTION_ID}/page-content`), { timeout: 5_000 });
  });

  test('navigating to FAQs page renders the faqs view', async ({ page }) => {
    await page.getByRole('link', { name: /faqs/i }).click();
    await expect(page).toHaveURL(new RegExp(`/dashboard/collections/${COLLECTION_ID}/faqs`), { timeout: 5_000 });
  });
});
