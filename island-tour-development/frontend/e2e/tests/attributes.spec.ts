 ...
 *
 * Covers /dashboard/attributes (list) and /dashboard/attributes/new (create),
 * and /dashboard/attributes/:key/edit (edit).
 *
 *  1. List page
...
test.describe('Attributes — list page (/dashboard/attributes)', () => {
  ...
  test('"Add Attribute" button is visible and navigates to /new', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/dashboard\/attributes\/new/, { timeout: 5_000 });
  });
  ...
  test('edit icon link navigates to the attribute edit page', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/dashboard\/attributes\/boat_type\/edit/, { timeout: 5_000 });
  });
...
test.describe('Attributes — create form (/dashboard/attributes/new)', () => {
  ...
  test('happy path: valid form submits via POST and navigates back to /attributes', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/dashboard\/attributes$/, { timeout: 8_000 });
  });
});
...
test.describe('Attributes — edit form (/dashboard/attributes/boat_type/edit)', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto('/dashboard/attributes/boat_type/edit');
    await page.waitForSelector('form', { timeout: 15_000 });
  });
