...
import { DEMO_TOUR_REF, dayOffset, isoWeekday, log, prisma, section, timeOf, today } from './_shared';
...
export async function seedAvailability(): Promise<void> {
  ...
  const alreadySeeded = await prisma.availabilitySchedule.count({ where: { tour: { reference: DEMO_TOUR_REF } } });
  if (alreadySeeded > 0) {
    log('Availability already seeded — skipping (run :clean first to rebuild).');
    return;
  }
  ...
  for (const [idx, tour] of tours.entries()) {
    ...
    const sched = await prisma.availabilitySchedule.createMany({ data: scheduleData, skipDuplicates: true });
    ...
    if (idx % 4 === 0) {
      exceptions.push({ tourId: tour.id, date: dayOffset(closeDateOffset), startTime: null, type: AvailabilityExceptionType.CLOSE_DATE, capacity: null, note: 'Annual maintenance / crew day off' });
    }
    if (idx % 5 === 0) {
      exceptions.push({ tourId: tour.id, date: dayOffset(capDateOffset), startTime: timeOf(startTimes[0]), type: AvailabilityExceptionType.SET_CAPACITY, capacity: Math.max(2, Math.floor(capacity / 2)), note: 'Reduced capacity (second boat in service)' });
    }
    if (exceptions.length) {
      const ex = await prisma.availabilityException.createMany({ data: exceptions, skipDuplicates: true });
      exceptionRows += ex.count;
    }
    const closedDates = new Set(exceptions.filter((e) => e.type === AvailabilityExceptionType.CLOSE_DATE).map((e) => (e.date as Date).getTime()));
    const capOverrides = new Map(
      exceptions
        .filter((e) => e.type === AvailabilityExceptionType.SET_CAPACITY && e.capacity != null)
    ...
    if (departureData.length) {
      const dep = await prisma.departure.createMany({ data: departureData, skipDuplicates: true });
      departureRows += dep.count;
    }

    await prisma.tour.update({ where: { id: tour.id }, data: { isBookable: true, availabilityConfirmedAt: new Date(), spotsRemaining: capacity } });
  }

  log(`Availability: ${schedRows} schedules, ${exceptionRows} exceptions, ${departureRows} departures across ${tours.length} tours.`);
}
