...
export async function seedEntityContent(): Promise<void> {
  ...
  for (const h of hubs) {
    ...
    const hasSections = await prisma.hubContentSection.findFirst({
      where: { hubId: h.id },
      select: { id: true },
    });
    if (!hasSections) {
      const sections: {
        ...
        },
        {
          type: HubSectionType.DISCOVER,
          heading: 'What to do',
          body: 'Snorkel the wrecks, swim with turtles, walk to the old lighthouse, or simply claim a patch of sand and relax.',
          order: 1,
        },
        {
          type: HubSectionType.LOCAL_TIP,
          heading: 'Local tip',
          body: 'Bring water shoes — the shoreline near the wrecks can be rocky. And book the earliest departure for the calmest seas.',
          order: 2,
        },
        {
        ...
        },
        {
          ...
          body: 'It is the closest thing to a deserted-island day you can book and still be home for dinner.',
          order: 4,
        },
      ];
    ...
    if (hubTours.length) {
      ...
      for (let i = 0; i < hubTours.length; i++) {
        ...
        const pick = await prisma.hubOurPick.upsert({
          ...
          create: {
            hubId: h.id,
            tourId: hubTours[i].id,
            pickType: pickTypes[i % pickTypes.length],
            description: desc,
            displayOrder: i,
          },
        });
      ...
      // Comparison group.
      const hasGroup = await prisma.hubComparisonGroup.findFirst({
        where: { hubId: h.id },
        select: { id: true },
      });
      if (!hasGroup && hubTours.length >= 2) {
        ...
        await prisma.hubComparisonGroupTranslation.createMany({
          data: NON_EN_LOCALES.map((locale) => ({
            groupId: group.id,
            locale,
            groupName: stub(locale, 'Relaxed vs Full-day trips'),
          })),
          skipDuplicates: true,
        });
