 ...
 * but the money only actually moves when `executeRefund` writes a SUCCEEDED
 * REFUND row (which is a no-op when Stripe is unconfigured or the charge was
 * never a real PaymentIntent). So a cancelled booking can sit PENDING with the
 * money still held - the dashboard must show that, not a false "refunded".
 *
