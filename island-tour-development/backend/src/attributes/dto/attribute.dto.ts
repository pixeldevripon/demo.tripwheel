...
export class AttributeDefinitionResponseDto {
  ...
  @ApiPropertyOptional({
    example: ['catamaran', 'yacht', 'speedboat'],
    nullable: true,
  })
...
export class TourAttributeResponseDto {
  ...
  @ApiProperty({
    example: 'catamaran',
    description: 'Scalar string, or JSON array string for ENUM_MULTI',
  })
  value!: string;
...
export class FilterItemDto {
  ...
  @ApiProperty({
    type: [FilterValueCountDto],
    description:
      'Present values + counts (ENUM/ENUM_MULTI/BOOLEAN); [] for numeric/text',
  })
...
export class FiltersResponseDto {
  ...
  @ApiPropertyOptional({ type: RangeDto, nullable: true })
  priceRange!: RangeDto | null;
  @ApiPropertyOptional({ type: RangeDto, nullable: true })
  durationRange!: RangeDto | null;
...
export class AttributeDefinitionQueryDto {
  @ApiPropertyOptional({
    description:
      'Filter by category slug (returns that category-specific set + global)',
  })
  @IsOptional()
  @IsString()
  category?: string;
  ...
  @ApiPropertyOptional({
    description: 'Only globals (no category-specific attributes)',
  })
  @IsOptional()
  @Transform(({ value }) =>
...
export class CreateAttributeDefinitionDto {
  ...
  @ApiPropertyOptional({
    ...
    description: 'Required for ENUM / ENUM_MULTI',
  })
  @IsOptional()
  @IsArray()
  @IsString({ each: true })
  ...
  @ApiPropertyOptional({
    type: [String],
    example: ['boat-tours'],
    description: 'Category slugs; omit/[] = global',
  })
  @IsOptional()
  @IsArray()
  @IsString({ each: true })
...
export class UpdateAttributeDefinitionDto {
  ...
  @ApiPropertyOptional({
    type: [String],
    example: ['catamaran', 'yacht', 'glass_bottom'],
  })
  @IsOptional()
  @IsArray()
  @IsString({ each: true })
...
export class AttributeValueItemDto {
  ...
  @ApiProperty({
    example: 'catamaran',
    description:
      'Scalar value as a string. For ENUM_MULTI use a comma-separated list, e.g. "turtles,coral".',
  })
  @IsString()
  value!: string;
}
