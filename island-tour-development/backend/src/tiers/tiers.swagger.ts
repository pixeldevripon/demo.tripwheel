...
export const ApiRequestSpotlightDocs = () =>
  applyDecorators(
    ApiOperation({
      ...
      description: 'Eligibility-gated (>=10 reviews, rating >=4.5) → REQUESTED.',
    }),
...
export const ApiGetTourSpotlightDocs = () =>
  applyDecorators(
    ApiOperation({ summary: 'Get a tour’s current + historical Spotlight requests' }),
...
export const ApiListSpotlightQueueDocs = () =>
  applyDecorators(
    ApiOperation({
      ...
      description: 'Filter by destinationId / status; activeCount is APPROVED+ACTIVE per destination.',
    }),
...
export const ApiApproveSpotlightDocs = () =>
  applyDecorators(
    ApiOperation({
      ...
      description: 'Sets the active window; re-checks eligibility and the max-3-per-destination cap.',
    }),
    ApiOkResponse({ type: SpotlightRequestResponseDto }),
    ApiNotFoundResponse({ type: NotFoundErrorDto }),
...
export const ApiRejectSpotlightDocs = () =>
  applyDecorators(
    ApiOperation({ summary: 'Reject a Spotlight request (sets rejectionReason)' }),
    ApiOkResponse({ type: SpotlightRequestResponseDto }),
    ApiNotFoundResponse({ type: NotFoundErrorDto }),
    ApiConflictResponse({ type: ConflictErrorDto }),
    authErrors(),
