...
- [~] **Resource layer (physical-asset correctness).** Two tours sharing one boat/fleet/guide are two `departures` rows with independent counters, so BOTH can sell to capacity - proven on live rows (Private Jet Ski Island Tour + Palm Beach Jet Ski Safari, same operator, same 09:00, 4 machines). Not a race: different primary keys, so no isolation level catches it. Industry-standard fix (FareHarbor/Bokun/Rezdy/Peek Pro/Checkfront all model Resources). **Phase 1 SHIPPED 2026-08-02:** `resources` + `tour_resources` (2 modes: EXCLUSIVE, EXCLUSIVE_ON_FIRST), pure `consumed()`/`claimant()`/`effectiveCapacity()` in `src/resources/resource-allocation.util.ts` (26 tests), migration purely additive with zero rows, NOT wired into booking, plus the capacity-reader audit. Phases 2-8 pending. Design frozen in `02-architecture/RESOURCE-ARCHITECTURE-REVIEW-VERDICT.md`; reader list in `02-architecture/CAPACITY-READER-AUDIT.md`
- [x] iCal export feed of departures for operators, and optional iCal import writing `availability_exceptions` (never mutating capacity directly) with `ical_sync_logs` — **BOTH HALVES SHIPPED**. **Export (2026-07-29):** `calendar_feeds` + `backend/src/calendar-feeds/` + shared RFC 5545 writer `src/common/ics/ics.util.ts` (the traveller booking `.ics` now projects onto it); two tokenized, rotatable, revocable feeds per operator — `BOOKINGS` (needs `VIEW_BOOKINGS`) and `DEPARTURES` (needs `MANAGE_AVAILABILITY`); `@Public()` `GET /calendar-feeds/:token/calendar.ics` with ETag/304 (DTSTAMP pinned to the data mtime, or the feed never caches); cancellations published as `STATUS:CANCELLED` rather than dropped. **Import (2026-08-02):** `calendar_subscriptions` + `calendar_events` + `ical_sync_logs` + `calendar_conflicts`, `backend/src/calendar-sync/`, SSRF-hardened fetcher with IP-pinned sockets (`src/common/net/`), `ical.js` parser, four operator-chosen import modes (`WARN_ONLY` default — iCal carries no seat count, so one external booking must not close a 60-seat boat), 15-min BullMQ repeatable poll with a stuck-sync reaper, conflict + failure inbox events, URL encrypted at rest. **Only a successful parse may remove a block**; disconnect defaults to KEEPING the dates. Full detail in `02-architecture/AVAILABILITY-AND-DEPARTURES.md` §9a (export) and §9b (import); PRD reconciliation in `02-architecture/ICAL-PRD-GAP-ANALYSIS.md`; **setup, routing and operator walkthrough in `02-architecture/ICAL-SETUP-AND-USAGE.md`**. Also shipped: a per-tour zero-PII `CHANNEL` export feed (the only kind safe to hand an OTA - publishes a date only when the tour cannot take another booking), operator **email** on conflict/failure/recovery alongside the inbox bell (transition-based, never per retry), and a real-database e2e proving imported blocks reach real departures
...
- [x] Platform PSP switch `PATCH /settings/payment-provider` — activation contract enforced by `missingProviderCredentials()`: Stripe needs `publishableKey` + `secretKey` + `webhookSecret` (the publishable key is required, the checkout refuses the payment step without it), Mollie needs `apiKey`; the 400 names **every** gap at once. Mirrored in the dashboard (`lib/settings/payment-requirements.ts`) so the confirm dialog **collects** the missing fields and saves them before switching, instead of failing into a toast. Never retroactive — webhooks/refunds route by the `Payment` row's own provider. See `02-architecture/BOOKING-AND-PAYMENTS.md` §8
- [x] Every credential input on the payments screen is autofill-guarded (`off` on text, **`new-password`** on secrets — `off` does not stop Chrome filling a password input when it holds a saved login, which was writing the admin's own email/password into the Stripe keys)
