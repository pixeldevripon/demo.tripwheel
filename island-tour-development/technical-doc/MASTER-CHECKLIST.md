...
- [~] Autocomplete rules (min 2 characters, 250ms debounce, grouped Categories & Hubs / Tours / Collections, CMS-driven zero-state and rotating placeholders) — typeahead ships; the **zero-state panel is DONE on the destination hero** (2026-08-05): focusing the empty field opens the same panel with two groups, Categories & Hubs (hubs first) then Collections, built from the island's already-gated lists so every row opens a page that renders. It is an addition to the typed panel, not a second component (`SearchTypeahead` branches on a query under 2 characters). Group headings are dictionary keys in all 7 locales. Still unverified: the rotating per-destination placeholders, and a zero state for the unscoped NAVBAR search
