...
- [x] **Calendar disables + hover hints** - no-schedule / closed / sold-out days blocked with reason tooltip (new `calendarNoDepartures`, `calendarClosed` dict keys, 7 locales); tooltip animates via `AnimatePresence` + `crossFade`.
    - **One tooltip, not two (2026-08-15, Pastel #79 / issue #219)** - the hint was written onto the cell twice: as the styled bubble AND as a `title` attribute, which the browser drew as its own plain tooltip below the cell. Every hovered date showed the same two words in two designs at once. The `title` is removed; the bubble is the only visible tooltip. Nothing was lost with it - the reason already reached assistive tech through the cell's `aria-label` (`"10, Sold out"`), which is what a `title` on a **disabled** button could never do reliably anyway. Guarded by `booking-calendar.test.tsx`, which asserts the popover contains no `[title]` at all: a `title` is invisible to a text query, so only an attribute assertion can catch it coming back. The client's report also confirmed the Sold out / Not running closure reason now flows end-to-end from the operator's portal button to the calendar cell.
...
- [x] **Mobile sticky CTA bar (2026-08-06, Pastel #37 / issue #90)** - below `lg` the card is not a sticky rail: it sits in the flow after the gallery and scrolls away, taking the price and the only way to book with it. `booking-sticky-bar.tsx` puts them back once the card has passed above the viewport - headline price + free-cancellation line left, CTA right - and hides again when the card returns. The card itself does not move. Notes worth keeping:
    - **`IntersectionObserver` on the card, plus a direction test.** The card is equally "not intersecting" on the way DOWN to it at the top of the page, so `!isIntersecting` alone made the bar greet every arrival. `rootMargin: -64px` starts the root's top edge below the fixed navbar.
    - **Portalled to `document.body`.** The card is wrapped in a framer `MountReveal`, and an animating ancestor's `transform` becomes the containing block for `position: fixed` descendants - the bar would be pinned to the bottom of the card for the length of the entrance.
    - **It ships its own spacer**, a flow element in the same portal (so it lands after the footer) measured from the bar with a `ResizeObserver`. Without it a fixed bar covers the last 61-105px of the document, which is the footer.
    - **The row wraps by language, not by breakpoint.** "Check availability" is 18 characters in EN and 27 in NL (`Beschikbaarheid controleren`); the row that fits both halves at 390px in EN clipped the cancellation promise to "Cancelación gratuita hasta…" in ES. `flex-wrap` + a `basis-44` price block drops the button onto its own full-width row exactly where it no longer fits (EN 61px at 390px, ES/PT/FR/NL/DE 105px). No media query can express this.
    - **CTA behaviour is shared with the inline button** via `hooks/tours/use-booking-cta.ts` - one checkout-query builder, so the two cannot drift. Ready -> checkout; not ready -> smooth-scroll the card back (`scroll-mt-20` clears the navbar) and focus the date trigger (`data-booking-date-trigger`, `preventScroll`).
    - **Founder calls (2026-08-06):** no save heart (Pastel #37 offered one "if it still looks clean"; the row is the page's tightest and five locales already take it to two lines - the heart stays on the hero photo per Pastel #36), and no check glyph on the cancellation line (a single line under a price is not a list; the glyph is what makes the card's trust ROWS scan as a set).
    - Gated off exactly where the inline CTA is: `bookingBlocked` (`operator_full`) and `availabilityDeadEnd`.
- [x] **The widget stopped doing its own FX (2026-08-06, Pastel #41 / issue #94)** - it built its price estimate by multiplying each source price by `money.fxRate` itself, and its arithmetic disagreed with the backend's in two visible ways. A $139 tour reached the tour CARD as the backend's whole "128" and the widget as its own `139 x 0.92 = 127.88`, so one page showed two prices for one tour; and the add-on and band rows carried cents on a platform where every other traveller-facing amount is a whole unit (a $39 add-on as "EUR 35,88").
    - **Backend serves them.** `FxRatesService.attachDetailMoney` (new) replaces `attachMoney` on the tour-detail path: same headline figures, plus `money.extraPersonPrice` and `money.ageBands` / `money.addOns` / `money.pickupLocations` keyed by child id. Every value goes through `retailWhole` (ceil to a whole currency unit) - the same rule the quote and the booking totals already used. `TourDetailMoneyDto` / `types/money.ts#TourDetailMoney`.
    - **Frontend renders what it is served.** `buildTourBookingData`'s `served()` prefers the backend value for the headline, bands, add-ons, pickup zones and the extra-guest surcharge. Its client-side `conv` survives ONLY as the fallback for a payload with no `money`: a same-currency shopper, an FX outage serving source prices at rate 1, or a `'use cache'` entry written before these fields existed (which is exactly what a stale EN page showed mid-verification while a freshly-fetched ES page was already correct).
    - **The quote's breakdown lines too.** `lines[].unitPrice`/`lineTotal` were 2dp, so the widget rendered "Adult x 1 x EUR 127,88" directly beneath its own "EUR 128" total. These are the human-readable explanation of the price - the charged figures come from `pricing` and are untouched. A line total is ceiled from the true amount rather than as `ceil(unit) x quantity`, so the rows still reconcile with the total.
    - **NOT applied to** operator cost (`priceNet`) or the EUR commission snapshot: neither is shown to a traveller, and inflating them would distort payout and revenue figures.
    - Verified on the Full-Day Catamaran ($139, rate 0.92): headline EUR 128, Adult EUR 128, Child EUR 77, Open bar EUR 23, GoPro EUR 36, total EUR 128 = 39 today + 89 later.
- [x] **Trust lines read as clickable (2026-08-06, Pastel #42 / issue #95)** - both phrases that open a policy modal were `font-semibold`, the same weight as the sentence around them, leaving a `decoration-it-border` (#e5e7eb) underline on white at 13px to carry the whole signal. The client read them as static text. `TrustRow` is now `font-normal` and `TrustLink` `font-bold` (one step up, the client's own suggestion), the underline is `decoration-it-ink-muted` and shifts to `it-primary` on hover, and a keyboard `focus-visible` ring was added (2px `it-primary`, 2px offset - it had none). Weight AND underline together, never colour alone. Each button gained `aria-haspopup="dialog"` and an accessible name of "{visible phrase}, {policyLinkHint}" - appended after the visible text, never instead of it, so voice control still matches what is on screen. New dict key `policyLinkHint` ("view policy"), 7 locales. Locale-proof by construction: the copy is split on `{link}`, and `zh` puts the phrase at the END of its sentence.
- [x] **Checkout stopped asking for a postal code (2026-08-15, Pastel #82 / issue #222)** - the Card panel carried a required "Postal code (locale-aware)" field. Two faults in one line. The label shipped an implementation note to the traveller, and the field itself contradicted the master: billing data (`billing_country` / `billing_postal_code` / `billing_city`) is pulled off the Stripe payment method at webhook time precisely so the booking form stays friction-free (master 3, "Billing data uit Stripe: geen extra friction op booking form"; `BOOKING-AND-PAYMENTS.md` §Billing). The input, its `postal` state and its required rule are gone from `checkout-payment.tsx`; **name on card is now the only field of ours in that panel** (and carries the asterisk the postal code used to - `handleReserve` has always rejected it empty, and the contact step marks every required field). No backend change was needed - `ContactDto.postalCode` was already optional and unsent, and `payments.service.ts` already reads `charge.billing_details.address.postal_code`. Notes:
    - **Nothing collects a postal code now, and there is no drop-in Stripe field that would.** THE fact to know before anyone re-opens this: **Stripe ships no standalone postal-code Element.** Verified twice - against the docs, and against the installed package: `StripeElementType` in `@stripe/stripe-js` 9.x lists `address · auBankAccount · card · cardNumber · cardExpiry · cardCvc · contactDetails · currencySelector · expressCheckout · iban · payment · paymentForm · …` and has **no `postalCode` member**. The split `CardNumber`/`CardExpiry`/`CardCvc` trio this panel uses therefore cannot render one at all. So `billing_postal_code` stays null unless Stripe supplies it (Link / wallet), costing one optional Enhanced-Conversions match signal and one AVS signal.
        - **The combined `card` Element was tried the same day and REVERTED.** It is the only Element that renders the locale-aware postal field the master's payment-section line means (`hidePostalCode: false`; it reads the issuing country off the card number, so a US card must give five digits and a UK card takes alphanumeric). But it merges number/expiry/CVC into ONE field, and in this panel it rendered as card-number-only with nothing else visible - the three labelled boxes the Figma draws were gone. Founder: "everything was fine before edits". Restored to the split Elements. **Do not re-try the combined Element without a design decision first.**
        - **The two real options if the postal code is wanted back**, both bigger than a field swap: (a) the **Payment Element**, which renders number/expiry/CVC as separate fields AND a postal row - but takes over the payment-method list below (our Card/iDEAL/PayPal rows with their brand marks and tinted selection); or (b) our own input again, which is exactly what Pastel 82 rejected.
        - **`billing_details.address` still carries only the contact step's country** (dropped entirely when empty rather than sent as `{country: undefined}`).
    - **Leaked-annotation sweep (the issue's third ask).** One other internal note was sitting in a customer-facing label slot: `filterModal.ratingsNote` = "(hidden until reviews exist)", which describes the `hasReviews` gate in `tours-filter-modal.tsx` rather than addressing a traveller. Never rendered, but declared on the component's dict type and one JSX line from shipping - removed from all 7 locales and from `ToursFilterModalDict`. No other parenthetical in `en.json` is an internal note (`(optional)`, `(local time)`, `(From {price} p.p.)`, `(KvK 169950)` all read to the customer). `DICTIONARY_VERSION` bumped; the checkout e2e now asserts the postal input is absent instead of filling it.
    - **Two stale e2e selectors fixed while verifying.** `completeContactStep` and two contact tests still typed into `getByLabel(/full name/i)`, but the contact form split the name in two long ago - so every test that called the helper, including both payment-step tests and the pay-through, was failing on a selector that matches nothing. Now first/last name; the four contact-step tests pass (two were red).
- [x] **Every payment method shows collapsed, none pre-selected (2026-08-15, Pastel #84 / issue #224)** - Card arrived selected AND expanded, so its form pushed iDEAL and PayPal below the fold. The client watched a test booker who had no card find iDEAL only much later: a conversion loss that never read as a bug, because the page was doing exactly what it was written to do. `checkout-payment.tsx` now starts at `method = null`, so all three rows render collapsed and a traveller sees what they can pay with before choosing one. Notes:
    - **"Which method?" became a real validation state.** Pressing Pay with nothing chosen used to be unreachable; it now answers with a form-level prompt in the existing error slot under the rows (new `selectMethodError`, 7 locales, `DICTIONARY_VERSION` bumped) rather than swallowing the click - the same rule the widget's Check Availability follows (2026-07-19 above). Picking a method clears it. FIELD errors are deliberately NOT cleared: the card inputs survive the switch, so their errors are still true when the traveller comes back.
    - **The Card Elements moved from "mount with the panel" to "mount on first pick"** - which the issue explicitly allows, and which spares an iDEAL/PayPal payer the Stripe iframe work entirely. That put them back in the path of the mount-into-a-still-animating-box defect, so `MethodRow` expands with the shared `Collapse` at `instant={open}` (snap OPEN, animate CLOSED) instead of its own `AnimatePresence` - the same contract the payment section itself uses in `checkout-form`, and the reason that prop exists.
    - **The card panel is never unmounted once opened** (`cardPanelMounted` latch + `MethodRow`'s `mounted` prop). Unmounting DESTROYS a Stripe Element, so a half-typed card number would not survive a look at PayPal; this is the entry-survival contract the contact step's Edit round trip already honours.
    - The method list is a `role='group'` labelled by the "Select a payment method" heading, which previously labelled nothing.
    - **Guarded twice.** `checkout-payment.test.tsx` (6 tests, no PSP needed - the ABSENT Stripe Element is the load-bearing assertion, since a row can look collapsed while its content sits in the DOM), and the checkout e2e, which now asserts none-selected + the prompt and picks Card before touching the card fields. The e2e half only runs where a PSP key is configured, which is why the component test exists.
    - **Not the Mollie panel:** it is card-only by founder decision - one always-selected row, no list to collapse.
    - **Related, not fixed here:** #223 (iDEAL's eligibility test is inverted - it offers "Not available for EUR" on a EUR booking) is a backend fix, and #220 (MCK-20) adds the operator T&C checkbox to this same step.
...
- Widget lives in `frontend/components/frontend/tour/tour-booking-card/` (`TourBookingCard` + sections: `price-header`, `booking-calendar`, `departure-times`, `party-selector`, `spectators-panel`, `price-summary`, `booking-cta`, `policy-modal`, `sell-out-notice`, `booking-sticky-bar`). State/math: `frontend/lib/stores/booking-store.ts` (`deriveBooking`); mapper: `frontend/lib/tours/booking.ts` (`buildTourBookingData`, `DUMMY_BOOKING_DATA`, `bookingUnitLabel`); hooks `frontend/hooks/tours/use-booking.ts` + `use-booking-cta.ts` (the shared primary action); provider `frontend/contexts/booking-context.tsx`.
...
- [x] **`pickupModel` / `pickupRequired`** - DONE 2026-07-25 (C1+C2 phase). Pickup stays a checkout (Step 2) field per master 5.8, now fully per spec: `PickupLocation.price` (per person, tour currency; charged ONLY on `pickupModel=PAID_ADDON`); checkout dropdown label "Pickup location (From $X p.p.)" with priced zone options "(+$X p.p.)" (no $0.00 decimals via `formatCheckoutMoney`), locked "No pickup, meet at location" default, and the "Other location, we'll confirm via WhatsApp" fallback; selecting a priced zone RE-QUOTES (`POST /bookings/quote` with `pickupLocationId`) and live-updates the summary Total/Pay today/Balance later + the payment CTA amount (`CheckoutLiveProvider`). `pickupRequired` enforced BOTH ends: checkout drops the "No pickup" option, starts on a "Select pickup location" placeholder and blocks Continue; reserve throws 422 without `pickupRequested` (zone or "other" both satisfy it). Pickup price snapshots onto the booking (`pickupUnitPrice`/`pickupTotalPrice`) and rides in `totalRetail`; quote emits a `kind:'pickup'` line. Dashboard Pickups tab is `pickupModel`-aware (price field on PAID_ADDON only; tab hidden entirely on NONE). `Ref:` master 5.8 · [Guide §9](./BOOKING-FLOW-DESIGN-GUIDE.md#9-pricing-and-commission-logic)

### 3.4 By booking model / timing

- [x] **`instantConfirmation`** - DONE 2026-07-25: renders as a notice card beneath the widget (`booking-notices.tsx`, dict keys x7) - deliberately NOT a trust-strip line (LD5 locks the strip to exactly two lines). [x] **`bookingType`** semantics: PRIVATE+UNIT charter exclusivity was already live end-to-end (whole-departure claim + `privateCharter` badge in `price-header.tsx`). `Ref:` master §6.1
- [x] **`bookingCutoffMinutes`** - consumed via the availability reads (calendar days + `/availability/check` slots are cutoff-filtered server-side; the §4 wiring removed the hardcoded slots) and now ALSO enforced at `POST /bookings/quote` (422 past cutoff, matching reserve) so a quote can no longer manufacture a dead-end checkout. `Ref:` [Availability §4](../02-architecture/AVAILABILITY-AND-DEPARTURES.md#4-read-contract)

### 3.5 Add-ons

- [x] **`addOns` rendered + priced end-to-end** - DONE 2026-07-25. Widget: "Show extras" consent toggle (collapsed by default, founder decision; nothing pre-checked per master ethical CRO) expands per-add-on stepper rows (name, description, price with `/per person` / `/per booking` suffix, capped at `maxQuantity`); chosen extras enter the optimistic totals, every quote (`addOns[]`), the checkout URL (`addons=id:qty`), and the reserve payload -> `BookingAddOn` snapshot rows. Backend caps quantity at `maxQuantity` (422). **Money rule (founder 2026-07-25): extras (add-ons + priced pickup) are EXCLUDED from the deposit-% and commission-% bases, and on the deposit models they ride the operator-collected balance in full** - deposit/pay-today = tour x pct only, balance = tour - tour x pct + extras, commission = rate x tour-only EUR; PAID_IN_FULL still charges everything today; `totalRetail`/`totalEur` stay the full booking (supersedes the LD18 "purchaseable in advance" reading). Dict keys x7: `showExtras`, `addOnsTitle`, `perBookingShort`. `Ref:` master E.3 / LD18 "purchaseable in advance"
- [x] **TYP lists the purchased extras** (2026-07-25, user request): `ThankYouResponseDto.addOns` (BookingAddOn snapshots: name/unit/quantity/unitPrice/totalPrice, non-identifying so visible unverified too, like `party`); TOUR DETAILS card renders an "Extras" row ("2 x Open bar upgrade", one line each; dict key `thankYou.extras` x7). Note: the TYP Pickup row shows the snapshotted pickup ADDRESS (master E.8), the checkout dropdown shows the zone NAME - junk demo zones with mismatched name/address read confusingly but are data, not a bug.

---

- [x] **Checkout summary shows the widget-style line breakdown** (2026-07-25, user request): the Booking summary card renders per-line item math above Total ("Adult x 2 x $139", add-on rows, pickup row) - server-derived from the URL selection (`breakdownRows` in checkout/page.tsx), swapped for the LIVE quote lines when a pickup re-quote runs (`CheckoutLiveTotals.lines`). UNIT/free rows drop the "x $0" tail.

### 3.6 Selection persistence (checkout round-trip)

- [x] **Widget selection survives Continue -> checkout -> "Back to tour"** (2026-07-25, user request). `use-booking-selection-persistence.ts` snapshots date/time/party/extras to sessionStorage per tour (`it-booking-selection:{tourId}`, tab-scoped) and restores on mount via the store's `hydrateSelection` (re-validated against CURRENT tour data: unknown band/add-on ids dropped, quantities re-clamped, past dates ignored; the TIME re-selects only after the date's live slots confirm it still exists - a slot that sold out meanwhile stays unselected). **Design note: ONE effect does restore-then-`store.subscribe` against the store API directly - a separate reactive persist effect fires its first run with the pre-restore render's stale closure and clobbers the snapshot with defaults (StrictMode double-mount makes that race a certainty in dev; shipped first, fixed same day).** The CHECKOUT PICKUP selection shares the same key (`pickup` field, merge-written): restored on the checkout form's mount (zone re-validated; a saved "No pickup" ignored if pickup became required) and it re-fires the live re-quote so the summary money restores too. The confirmed payment handoff (`checkout-processing.tsx` goToTyp, tour id threaded via `&tour=` on the processing URL) clears the snapshot so a booked trip never restores. No-op in design/demo mode; storage failures swallowed.
