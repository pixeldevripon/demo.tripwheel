...
- Webhook endpoint: `POST /api/v1/payments/webhook` (Stripe), `POST /api/v1/payments/webhook/mollie` (Mollie, recorded only).
- Handled events: `payment_intent.succeeded`, `payment_intent.payment_failed`.
- Intent creation: `POST /api/v1/payments/bookings/:id/intent` (returns `clientSecret`,
  `publishableKey`, `paymentMethodTypes`).
- Code: `backend/src/payments/` (`stripe.service.ts`, `payments.service.ts`).
