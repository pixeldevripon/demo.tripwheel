...
- [x] **F9 - shed doomed requests.** DONE 2026-08-10. Reserve rejects certain doom (departure not open / zero seats free - seats-independent, so never a false reject against the row AS READ; the milliseconds-wide staleness window where a concurrent cancel frees a seat mid-request is covered by the advisory contract - identical outcome to clicking a moment earlier) before pricing, FX snapshots and the transaction; advisory by design, the guarded claim stays the only authority, error messages unchanged. The three public availability reads get a 15s in-process cache wired in the CONTROLLER - the booking path cannot consult it by construction; expiry is the invalidation at this window. Per-route @Throttle on POST /bookings deliberately NOT added: the per-departure 60/min limiter already bounds hold churn precisely, per-IP would punish hotel/NAT tour groups, and a route-level override disables the internal-key bypass that SSR prerenders and the loadtest rely on.
- [x] **F10 - resource-layer decision.** DECIDED + DONE 2026-08-10 (founder call): shared-resource capacity is not a launch requirement - migration `20260810140000_drop_orphaned_resource_layer` drops the orphaned `resources`/`tour_resources` tables + enums; schema matches code again. The cross-tour shared-boat gap is filed in the migration header as post-launch work.
