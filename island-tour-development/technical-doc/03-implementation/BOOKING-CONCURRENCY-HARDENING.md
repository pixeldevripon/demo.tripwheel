...
> **Status: COMPLETE — all eleven fixes landed 2026-08-10 (PR-1 #164 through the F11 truth-up), including the F7 baseline run and two orphaned-layer drops (resources, iCal) the work surfaced. The only successor task is the pre-VPS migration baseline squash.** Scope: the seat-claim/release
...
| [x] F6 | 🟠 P1 | Explicit pool + timeouts | **DONE 2026-08-10** - pool max `DB_POOL_MAX` (default 25), connect 5s / statement 10s / idle-in-txn kill 15s / lock wait 3s (native pg options, applied-to-connection proven by e2e `current_setting`); reserve txn opts explicit `{maxWait: 2s, timeout: 5s}`; 55P03 maps to 503 "try again" in BOTH adapter error shapes (raw P2010-nested and bare client-query DriverAdapterError - the second was probe-discovered, a one-shape predicate 500s on the insert path). e2e: `test/db-pool-config.e2e-spec.ts` incl. live held-lock shed at ~3s. Numbers are conservative starts - F7 tunes them | `prisma.service.ts`, `env.validate.ts`, `.env.example`, `bookings.service.ts` | S |
| [x] F7 | 🟠 P1 | Load test with postconditions | **DONE 2026-08-10** - k6 harness (`scripts/loadtest/`: hot/spread/mixed + SQL postcondition checker + runbook). Baseline run against F1-F6 code, local: hot-100/500/1000 all exact-capacity (20 claims, ledger match), zero unexpected 5xx across ~3,100 requests, hot p95 227-285ms, spread-500 all claimed (p95 970ms = pool-25 queueing), mixed reads never starved. Verdict layering as designed: 60/min limiter → 20x201 + 40x422 + rest 429. Numbers are correctness baseline, NOT capacity - re-record on the VPS before launch (runbook table keeps history) | `scripts/loadtest/` | M |
| [x] F8 | 🟡 P2 | Replica-safe sweeper | **DONE 2026-08-10** - all four @Cron sweeps became BullMQ job schedulers with fixed ids on the existing platform-jobs queue (option 1, the designed home); N replicas upsert one schedule each, one worker runs each tick (two-replica run verified). attempts=1 (the schedule is the retry), failures retained and visible. Outbox relay needed nothing - already jobId-deduped | `workers/platform-queue.ts:PLATFORM_SCHEDULES`, `nightly-jobs.service.ts`, processor | S |
| [x] F9 | 🟡 P2 | Shed doomed requests | **DONE 2026-08-10** - (1) reserve sheds certain doom (not-open status / zero seats free, seats-independent) BEFORE pricing + FX + txn, advisory by design, claim stays the authority, same messages; (2) 15s in-process cache on the three public availability reads, wired in the CONTROLLER so the booking path cannot consult it by construction. (3) per-route @Throttle on POST /bookings deliberately NOT added: the per-departure 60/min limiter is the precise churn bound already, per-IP punishes hotel/NAT tour groups, and a route-level override would disable the internal-key bypass the SSR prerender and loadtest depend on | `bookings.service.ts`, `availability.controller.ts`, `availability-read-cache.ts` | S/M |
| [x] F10 | 🟡 P2 | Resource-layer decision | **DECIDED + DONE 2026-08-10** - founder call: shared-resource capacity is NOT a launch requirement. Migration `20260810140000` drops the orphaned tables + enums (empty everywhere, `IF EXISTS` for post-squash replays); schema matches code again. The cross-tour shared-boat gap is FILED in the migration header as post-launch work - re-land from the reverted ADR when real, never resurrect the empty tables | migration `20260810140000_drop_orphaned_resource_layer` | decision |
| [x] F11 | 🟡 P2 | Documentation truth-up | **DONE 2026-08-10** - BOOKING-AND-PAYMENTS status header + REAL BookingStatus machine (incl. the public ON_HOLD checkout-abandon cancel the fact-check caught); operator_full settled as modeled-but-DISABLED-in-v1 with the founder rationale; EVENT-DRIVEN §3 order corrected to insert-first/claim-last with outbox writes scoped to confirm + FULL-refund cancel; AVAILABILITY-BOOKING-ARCHITECTURE §7 re-anchored to SYMBOLS with as-built deltas. Every claim fact-checked against prod source (26/30 letter-perfect, 4 corrected before merge) | `technical-doc/` | S |
