...
| [x] F5 | 🟠 P1 | DB CHECK constraint | **DONE 2026-08-10** - migration `20260810120000` (ledger-based repair of drifted rows + `NOT VALID`/`VALIDATE`). Capacity-edit guards already existed (`updateDeparture` floor → 400 + optimistic lock → 409; materializer never resizes booked rows), so no 23514 is reachable through a service writer - the constraint is purely the backstop it was meant to be, and no extra error mapping was added. NOTE: CHECK constraints cannot live in the Prisma schema DSL - this one exists ONLY in the raw migration, so any future migration re-baseline must carry it forward by hand | migration + `test/booking-concurrency.e2e-spec.ts` | S/M |
