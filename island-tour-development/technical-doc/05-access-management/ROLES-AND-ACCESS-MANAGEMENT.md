...
| Tours | `CREATE_TRIP` | `EDIT_TRIP` | `DELETE_TRIP` / `MANAGE_TRIPS` (admin). **Publishing is always Island Tours' (conflict #1, 2026-07-28)**: operators hold NO `MANAGE_TRIPS`; they `submit-for-review` (`EDIT_TRIP`, same readiness bar as publish) and pause/archive their own tours (downward = safe); publish/unpause/restore/approve/reject are `MANAGE_TRIPS`. **`submit-for-review` accepts `DRAFT`, `PAUSED` and `ARCHIVED` (2026-08-01)**: every exit from the parked states is `MANAGE_TRIPS`, so restricting the REQUEST to `DRAFT` left an operator with a paused tour able only to archive it and one with an archived tour able to do nothing - a dead end (operator test report §02). Submitting stamps `approvalStatus = PENDING` and never moves `status`, so asking widened, not deciding; `approve`/`reject` follow the same three statuses so a request can be decided wherever it was raised. `LIVE` is excluded - it is already published. `approvalStatus` gates publish (`NOT_SUBMITTED/PENDING/APPROVED/REJECTED`; LIVE implies APPROVED - an admin publish stamps it); reject requires an actionable note the operator sees. **The review loop has its own surface (dashboard #79, 2026-08-15)**: the admin Tours list (`GET /tours/admin/all`) excludes `PENDING`/`REJECTED` by default - those tours live on the `MANAGE_TRIPS`-gated `/submissions` queue (FIFO on `submittedAt`, `reviewLoop=true` = both states, `approvalStatus=ANY` = no filter for jump-to-anything surfaces); approval also realigns a never-published tour's slug to the final title (see SLUG-REGISTRY §5.5). Operator `my-tours` always shows the operator everything of their own |
