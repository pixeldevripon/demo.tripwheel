...
- **Wishlist heart on every card:** top-right overlay on the image on ALL viewports (founder decision 2026-08-13, overriding master §3.5's "mobile: bottom-right"); ~32px white circular backdrop with subtle shadow; outlined (default) → filled brand-orange (wishlisted); **optimistic UI** (fill immediately, revert on API failure); no page navigation.
...
| --------------- | ------------------------------------------------------------------------ | ------------------------------------------- | ----------------------------------------------------------------------------- | ------------------- |
| `operator_link` | Total · Pay today (deposit) · Balance later (operator sends secure link) | locked `Reserve my spot - Pay {deposit}`    | locked "Pay only {pct}% today, the rest later" + free cancellation | **yes**             |
| `on_arrival`    | Total · Pay today (deposit) · Balance on arrival                         | locked `Reserve my spot - Pay {deposit}`    | locked "Pay only {pct}% today, the rest later" + free cancellation | **yes**             |
| `paid_in_full`  | Total · Pay today = total (no balance row)                               | locked `Reserve my spot - Pay {total}`      | free cancellation only, no payment line (conflict log 81)                     | **yes**             |
...
- **Deposit uses the real `depositPct` from the tour**, never a constant, and **never rounded**: tier rates run 20-30 in steps of 2.5 (LD24), so `Math.round` turned a 27.5% tour into "Pay only 28% today" and put the widget half a percent away from the server quote (fixed 2026-08-06, Pastel #32).
- `requiresDeposit` (`lib/tours/booking.ts`) is the one definition of "this tour actually takes a deposit online" — a deposit model **and** `0 < depositPct < 100`. The store reads it; it used to recompute the same expression, and the copy drifted from it.
- `deriveBooking` derives per-model money rows, `balanceLabel` (**"Balance on arrival"** for `on_arrival`), and a `paymentTrust` line — present on the deposit models, `null` everywhere else. Dict keys (7 locales): `bookingUnavailable`, `balanceOnArrival`, `payOnArrival`; `payLater` is the LOCKED neutral "{link}, the rest later", shared by both deposit models. **`payOnArrival` was removed** alongside `payInFull` (2026-08-06, Pastel #32): naming how the balance is collected is the model-baking the client rejected - the rest is not always paid through a link. That detail belongs in the deposit modal (B.88), not on the commit-moment line. **`payInFull` was removed** (2026-08-06): it rendered a second trust line on `paid_in_full`, which conflict log 81 does not allow.
