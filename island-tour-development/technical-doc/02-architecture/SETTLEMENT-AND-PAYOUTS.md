...
- `paid_in_full`: scheduled **manual net payout** per operator per cycle, released **after the cancellation window closes / tour completion** (clawback-safe, and it matches the master's "revenue recognized on tour completion" and the affiliate on-hold-then-approve lifecycle).
  - **Payout HOLD on a pending cancellation request (built, B4/#89 follow-up):** the release sweep skips any `RECORDED` `paid_in_full` net whose booking has a pending cancellation request (`utc_cancellation_requested_at` set, `utc_cancelled_at` null). Refund entitlement is judged at the **request instant** (master 6.4), so a request made in-window still owes the traveller a refund even once an admin actions it *after* the window closes - releasing to the operator first would leave IT unable to claw it back. The hold lifts when the request resolves: **cancelled** -> booking leaves `CONFIRMED`, never eligible; **rejected** -> `utc_cancellation_requested_at` cleared, so the row releases normally on the next sweep. The settlements ledger surfaces this as a derived `payout_held` flag ("On hold - cancellation requested" in the dashboard, instead of "Ready to pay out"). Enforced in `SettlementsService.isPayoutEligible`.
  - **Settlement REVERSAL on cancel (built, #90):** once a booking is actually cancelled its settlement is voided - `status -> REVERSED`, `net_position -> 0`, `operator_payout -> null`. A cancelled booking delivers nothing, so the operator-payout obligation disappears. `amount_collected`/`commission_owed` stay as the historical confirmation record (audit trail); only the obligation is zeroed. Two paths: (1) `BookingsService.cancel` reverses inline the moment a confirmed booking is cancelled; (2) `SettlementsService.reverseStaleCancelledSettlements` runs hourly (BEFORE the release sweep) to self-heal rows written before this logic or a settle-just-after-cancel race. A `PAID_OUT` row is never reversed (money already moved = a manual clawback in v1). The pending-payout summary and the release cron both exclude `REVERSED`. Forfeit-to-operator on an after-window cancel is a deferred D-tail policy.
  ...
  - **Refund lifecycle fully wired to Stripe (built, #91):** the `REFUND` row carries Stripe's ACTUAL answer, not an assumed success. Cards refund synchronously (`succeeded` -> row SUCCEEDED); bank-debit methods (iDEAL/SEPA/ACH) are created `pending` -> row **PROCESSING** (booking still reads "Refund pending" - money has not landed). The webhook handles **`refund.updated` + `refund.failed`** and reconciles the row by `refund_id` with out-of-order-safe transitions: -> SUCCEEDED only from PROCESSING; -> FAILED from PROCESSING *or* SUCCEEDED (Stripe can fail a bank refund days later); a still-`pending` update is ignored. A FAILED refund logs loudly and flips the booking back to "Refund pending" for the ops queue. **Retry (pre-B6):** re-invoking the cancel action on an already-CANCELLED booking re-attempts an owed FULL refund; the retry advances the Stripe idempotency key (`refund-{id}-r{n}` per failed attempt) so Stripe's cache never replays the failed refund. Also hardened: `payment_intent.payment_failed` no longer touches `REFUND` rows (they share the charge's intent id). **Stripe dashboard config:** the webhook endpoint's event subscription must include `refund.updated` and `refund.failed` (local `stripe listen` forwards everything by default).
