...
- Add a `settlements` ledger that records, per booking, `amount_collected`, `commission_owed`, and `net_position` (the payout owed the operator). **Amended by founder decision 2026-07-26: ONLY `paid_in_full` bookings write a row** - the one model where Island Tours holds money it owes the operator. Deposit models are self-settling (LD24: the deposit IS the commission - nothing ever moves) and recording them was ambiguous noise; migration `20260726120000_settlements_paid_in_full_manual_payout` purged the old rows. *(Superseded original: "every model writes a row, even deposit models".)*
- `paid_in_full`: **manual net payout, confirmed row by row** (founder 2026-07-26). An admin transfers the money by hand and then clicks **"Mark as paid"** on the dashboard row (`PATCH /settlements/:id/mark-paid`, `MANAGE_BOOKINGS`; a `mark-unpaid` undo exists for mis-clicks). `PAID_OUT` always means a human confirmed the transfer - no cron ever flips it. The cancellation window still governs, but as *eligibility*: a row is only payable **after the free-cancellation (clawback) window closes / tour completion**, enforced server-side both in the UI flag (`payout_eligible`) and in the endpoint's guards. *(Superseded original: the hourly `releaseEligiblePayouts` auto-release cron - removed; its cron-flipped `PAID_OUT` rows were reverted to `RECORDED` by the same migration.)*
  - **Payout HOLD on a pending cancellation request (built, B4/#89 follow-up):** mark-paid is refused (409) and the row flagged for any `RECORDED` net whose booking has a pending cancellation request (`utc_cancellation_requested_at` set, `utc_cancelled_at` null). Refund entitlement is judged at the **request instant** (master 6.4), so a request made in-window still owes the traveller a refund even once an admin actions it *after* the window closes - paying the operator first would leave IT unable to claw it back. The hold lifts when the request resolves: **cancelled** -> booking leaves `CONFIRMED`, never eligible; **rejected** -> `utc_cancellation_requested_at` cleared, so the row becomes payable. The ledger surfaces this as a derived `payout_held` flag ("On hold - cancellation requested" in the dashboard, instead of "Ready to pay"). Enforced in `SettlementsService.isPayoutEligible` + `markPaidOut`.
  - **Settlement REVERSAL on cancel (built, #90):** once a booking is actually cancelled its settlement is voided - `status -> REVERSED`, `net_position -> 0`, `operator_payout -> null`. A cancelled booking delivers nothing, so the operator-payout obligation disappears. `amount_collected`/`commission_owed` stay as the historical confirmation record (audit trail); only the obligation is zeroed. Two paths: (1) `BookingsService.cancel` reverses inline the moment a confirmed booking is cancelled; (2) `SettlementsService.reverseStaleCancelledSettlements` runs hourly (`settlement-reverse-sweep`) to self-heal rows written before this logic or a settle-just-after-cancel race. A `PAID_OUT` row is never reversed (money already moved = a manual clawback in v1). The pending-payout summary and the mark-paid guard both exclude `REVERSED`. Forfeit-to-operator on an after-window cancel is a deferred D-tail policy.
  - **Refund status is ledger truth, never assumed (built, #90):** cancelling a booking *owes* a refund but does not itself move money - the refund only happens when `executeRefund` writes a `REFUND` payment (a no-op when Stripe is unconfigured or the charge was never a real PaymentIntent). `deriveRefundState` reads the payment ledger and returns `NONE | PENDING | PARTIAL | REFUNDED`; the dashboard (bookings list + cancellation queue) renders it, so a cancelled-but-not-yet-refunded booking reads **"Refund pending"** (money still held), never a false "Refunded". The settlement REVERSED chip therefore describes only the voided *operator payout*, not the traveller refund.
  - **Refund lifecycle fully wired to Stripe (built, #91; status unified 2026-07-26):** the `REFUND` row carries Stripe's ACTUAL answer, not an assumed success. Cards refund synchronously (`succeeded` -> row **REFUNDED**); bank-debit methods (iDEAL/SEPA/ACH) are created `pending` -> row **PROCESSING** (booking still reads "Refund pending" - money has not landed). **At the settle point the ORIGINAL charge row also flips SUCCEEDED -> REFUNDED** (and back on a late failure), so the payments list never shows a green "Succeeded" charge on a refunded booking; migration `20260726140000` backfilled rows written under the old SUCCEEDED convention. The webhook handles **`refund.updated` + `refund.failed`** and reconciles the row by `refund_id` with out-of-order-safe transitions: -> REFUNDED only from PROCESSING; -> FAILED from PROCESSING *or* REFUNDED (Stripe can fail a bank refund days later); a still-`pending` update is ignored. A FAILED refund logs loudly and flips the booking back to "Refund pending" for the ops queue. **Retry (pre-B6):** re-invoking the cancel action on an already-CANCELLED booking re-attempts an owed FULL refund; the retry advances the Stripe idempotency key (`refund-{id}-r{n}` per failed attempt) so Stripe's cache never replays the failed refund. Also hardened: `payment_intent.payment_failed` no longer touches `REFUND` rows (they share the charge's intent id). **Stripe dashboard config:** the webhook endpoint's event subscription must include `refund.updated` and `refund.failed` (local `stripe listen` forwards everything by default).
...
Row semantics per v1 model (amended 2026-07-26 - paid_in_full only):

| Model | Ledger row? | `amountCollected` | `commissionOwed` | `netPosition` | Action |
|---|---|---|---|---|---|
| `operator_link` | **no row** | - | - | - | self-settling (deposit = commission; nothing to move) |
| `on_arrival` | **no row** | - | - | - | self-settling (deposit = commission; nothing to move) |
| `paid_in_full` | yes, at confirmation | `total` | `commission` | `+ (total - commission)` | `RECORDED` (= payout due) until an admin **manually marks it paid** after the bank transfer -> `PAID_OUT` (sets `operatorPayout` + `settledAt`); payable only once the cancellation window closes |

- `net_position` is the payout owed to the operator; it is zeroed on `REVERSED` (cancelled booking).
- Exactly one settlement row per confirmed `paid_in_full` booking; the self-settling models write none.
...
- **Automate the `paid_in_full` payout:** the payout model is decided - a clawback-safe payout after the cancellation window closes. V1 executes it manually (admin bank transfer + "Mark as paid" on the ledger row); v2 automates it via a Stripe Connect destination charge (`application_fee_amount = commission`) with the transfer released on the same post-window schedule.
