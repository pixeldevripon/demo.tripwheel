    ...
    return (
        <SettingsCard
            ...
            <div className='grid gap-6 sm:grid-cols-2'>
                <TextField
                    label='Audience ID'
                    registration={register('audienceId')}
                    error={errors.audienceId?.message}
                />
                <TextField
                    label='Server Prefix'
                    registration={register('serverPrefix')}
                    error={errors.serverPrefix?.message}
                    placeholder='us21'
                />
            </div>
        </SettingsCard>
...
const PROVIDERS: ProviderEntry[] = [
    ...
    },
    {
        ...
        keyPlaceholder: 'key...',
        keyHint:
            'Key from console.mistral.ai (free experiment tier available).',
...
function ModelTierBadge({ tier }: { tier: 'free' | 'paid' }) {
    return (
        <span
            className={
                tier === 'free'
                    ? 'ml-2 rounded-full bg-success-subtle px-1.5 text-xs font-medium uppercase text-success-fg'
                    : 'ml-2 rounded-full bg-surface-inset px-1.5 text-xs font-medium uppercase text-content-muted'
            }>
            {tier}
        </span>
    );
        ...
        <SettingsCard
            ...
            <Field>
                ...
                <Select
                    value={provider}
                    onValueChange={v => {
                        setValue('translationProvider', v, {
                            shouldDirty: true,
                        });
            ...
            {hasModelList && (
                <Field>
                    ...
                    <FieldDescription>
                        Free = usable on the provider&apos;s free tier; Paid =
                        needs paid credits.
                    </FieldDescription>
                </Field>
            )}
...
const cookiebotSchema = z.object({ cookiebotCbid: z.string().optional() });
    ...
    return (
        <SettingsCard
            ...
            onSubmit={handleSubmit(v =>
                mutate({ cookiebotCbid: v.cookiebotCbid })
            )}
            isSaving={isPending}
...
function SocialWidgetsCard() {
    ...
    const {
        ...
        defaultValues: { enableWhatsappChat: false, whatsappNumber: '' },
    });
    ...
    return (
        <SettingsCard
            ...
            <CheckboxField
                ...
                onChange={c =>
                    setValue('enableWhatsappChat', c, { shouldDirty: true })
                }
            />
            <TextField
