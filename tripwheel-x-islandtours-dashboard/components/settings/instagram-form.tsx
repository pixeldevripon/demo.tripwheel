      ...
      <div className="space-y-6">
        ...
        <div className="flex max-w-2xl flex-wrap items-center justify-between gap-x-6 gap-y-3 rounded-lg bg-surface px-4 py-3 text-sm">
          {connected ? (
            <div className="flex flex-wrap items-center gap-x-8 gap-y-1.5">
              <span className="flex items-center gap-2">
                ...
                <span className="font-medium">
                  {formatWhen(connection?.lastSyncedAt ?? null) ?? 'Never'}
                </span>
              ...
              <span className="flex items-center gap-2">
                ...
                <span className="font-medium">
                  {formatWhen(connection?.tokenExpiresAt ?? null) ?? 'Unknown'}
                </span>
              </span>
            </div>
          ) : (
            <span className="flex items-center gap-2 text-content-muted">
              ...
              No token yet - paste one below and run a sync to connect.
            </span>
          )}
        ...
        <div className="grid gap-x-6 gap-y-4 sm:grid-cols-2">
          ...
          <Field>
            ...
            <FieldDescription>How often tiles auto-sync.</FieldDescription>
          </Field>
          ...
          <Field>
            ...
            <FieldDescription>How the public grid is drawn.</FieldDescription>
          </Field>
          ...
          <Field>
            ...
            <FieldDescription>Auto-filled from the account.</FieldDescription>
          </Field>
          ...
          <Field>
            ...
            <div className="flex h-9 items-center gap-2">
              <Checkbox
                ...
                checked={enabled}
                onCheckedChange={(c) =>
                  setValue('enabled', c === true, { shouldDirty: true })
                }
              />
              <Label
        ...
        {enabled && !handle && (
          <p className="flex items-start gap-2 text-xs text-danger-fg">
            ...
            />
            Without a handle the whole section stays hidden - it is the heading it
            is built around.
          </p>
        )}
      </div>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <CardDescription className="mt-1">
              Mirrored from the connected account. The first {slots} reach the
              page - that is the cap for the{' '}
              {account?.layout === 'GALLERY' ? 'gallery' : 'grid'} layout (grid{' '}
              {SLOTS_BY_LAYOUT.GRID}, gallery {SLOTS_BY_LAYOUT.GALLERY}). Reorder
              to choose which; hide any you would rather not show.
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center gap-2 rounded-md border border-dashed border-line py-16 text-content-muted">
            ...
            <p className="text-sm">
              No tiles yet - connect the account above and run a sync.
            </p>
          </div>
        ) : (
          <>
            {ordered.length < slots && (
              <p className="rounded-md bg-surface-inset p-3 text-xs text-content-muted">
                ...
                short until the sync brings in {slots}.
              </p>
            )}
            ...
            {ordered.length > slots && (
              <p className="rounded-md bg-surface-inset p-3 text-xs text-content-muted">
                ...
                {account?.layout === 'GALLERY' ? 'gallery' : 'grid'} layout draws
                only the first {slots}. The greyed-out tiles from #{slots + 1}{' '}
                down stay saved here - move one up to put it on the page.
              </p>
            )}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              It disappears from the destination pages. The next sync will bring
              it back if the post is still among the recent ones - hide it
              instead to keep it out for good.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <div className="relative aspect-384/337 bg-surface-inset">
        ...
        <span className="pointer-events-none absolute left-2 top-2 inline-flex items-center gap-1 rounded-full bg-n-1000/70 px-2 py-0.5 text-2xs font-medium text-n-0 tabular-nums">
          ...
          {hidden ? (
            ...
            <span className="font-normal">· not shown</span>
          ) : null}
        </span>
        ...
        <div className="absolute right-1.5 top-1.5 z-10 flex gap-1">
          ...
          >
            <HugeiconsIcon
              ...
              className="size-3"
            />
            <span className="sr-only">{hidden ? 'Show tile' : 'Hide tile'}</span>
          </button>
          <button
            type="button"
            onClick={onDelete}
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            The photo, caption and link come from Instagram and refresh on every
            sync. What you set here sticks.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <Field>
            ...
            />
            <FieldDescription>
              Overrides the caption for screen readers (hashtags and mentions
              make poor alt text).
            </FieldDescription>
          </Field>
