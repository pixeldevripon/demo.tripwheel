...
function SectionLocaleEditor({
    ...
    const { mutate: upsert, isPending } =
        useUpsertPageContentSectionTranslation(basePath, entityId);
        ...
        upsert(
            {
                groupId,
                locale,
                payload: { heading: values.heading, body: values.body },
            },
            {
                onSuccess: () =>
                    toast.success(`${LOCALE_LABELS[locale]} saved.`),
                onError: err =>
                    toast.error(
...
function SectionGroupCard({
    ...
    const { mutate: updateGroup, isPending: isUpdating } =
        useUpdatePageContentSection(basePath, entityId);
    const { mutate: deleteGroup, isPending: isDeleting } =
        useDeletePageContentSection(basePath, entityId);

    const byLocale = new Map(group.translations.map(t => [t.locale, t]));
    const en = byLocale.get(DEFAULT_LOCALE);
    const translatedCount = group.translations.filter(t =>
        t.heading?.trim()
    ).length;
            ...
            <CollapsibleCard
                ...
                title={
                    <span className='line-clamp-1'>
                        {en?.heading ?? '(no English heading)'}
                    </span>
                }
                meta={
                    <span className='flex flex-wrap items-center gap-1.5'>
                        {en?.body && (
                            <span className='line-clamp-1 max-w-96 text-xs font-light text-content-muted'>
                                {en.body}
                            </span>
                        )}
                        <StatusBadge
              ...
              already had to fix). */}
                    {CONSOLE_TYPE_BY_BASE[basePath] && (
                        <p className='text-xs text-content-muted'>
                            Translate this section into the other languages in
                            the{' '}
                            <Link
            ...
            <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This deletes the section and all of its
...
function AddSectionForm({
    ...
    const { mutate: create, isPending } = useCreatePageContentSection(
        basePath,
        entityId
    );
    ...
    function onSubmit(values: AddSectionValues) {
        create(
            ...
            },
            {
                onSuccess: () => {
                    toast.success(
                        'Section added. Expand it to add translations.'
                    );
                    reset({ heading: '', body: '' });
                },
                onError: err =>
...
export function ContentSectionManager({
    ...
    const {
        ...
        isError,
        error,
    } = usePageContentSections(basePath, entityId);
    const list = groups ?? [];
        ...
        <div className='space-y-4'>
           ...
           truth, and invites an admin to re-type copy that already exists. */
                ...
                <div className='flex flex-col items-center gap-3 py-16 text-muted-foreground'>
                    ...
                    <p className='text-xs'>
                        The public page falls back to its built-in labels until
                        you add one.
                    </p>
                </div>
            ) : (
