...
// ── Social sharing (category OG image) ───────────────────────────────────────
...
function SocialCard({ category }: { category: CategoryDetail }) {
  const { mutate: updateCategory, isPending } = useUpdateCategory();

  const { handleSubmit, control, reset } = useForm<SocialValues>({
    resolver: zodResolver(socialSchema),
    defaultValues: { ogImage: category.ogImage ?? '' },
  });

  useEffect(() => {
    reset({ ogImage: category.ogImage ?? '' });
  }, [category, reset]);

  function onSubmit(values: SocialValues) {
    updateCategory(
      { id: category.id, payload: { ogImage: values.ogImage || null } },
      {
      ...
      <CardContent className="pt-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          <Field>
            ...
            />
            <FieldDescription>
              Image shown when this category is shared on social media (the Open Graph image).
...
// metaTitle / metaDescription live on CategoryPageContent; saved via a partial
  ...
  return (
    <div className="border border-border bg-muted/30 px-4 py-3 space-y-1">
      ...
      <p className="text-[18px] leading-[1.3] text-[#1a0dab] truncate">
        {title || `${categoryName || 'Category'} Tours & Activities`}
      </p>
...
function MetaLocalePanel({ category, locale, isEnglish = false }: MetaLocalePanelProps) {
  const { data: content, isLoading } = useCategoryPageContent(category.id, locale);
  const { data: translation } = useCategoryTranslationByLocale(category.id, locale);
  ...
  // English falls back to the category's canonical name/overview; other locales
  ...
  const localizedOverview = isEnglish
    ? translation?.overview || category.overview || ''
 ...
 {...register('metaTitle')}
        ...
        <div className="flex items-center justify-between gap-3">
          <FieldDescription>
            Pre-filled from the {isEnglish ? 'category name' : 'translated name'}. Edit to customize.
          </FieldDescription>
          <button
            type="button"
...
export function CategorySeoTab({ category }: CategorySeoTabProps) {
 ...
 <MetaLocalePanel category={category} locale='en' />
        <p className="mt-4 text-xs text-content-muted">
          English only here - translate into the other languages in the{' '}
          <Link
            href={`/translations/category/${category.id}/es`}
