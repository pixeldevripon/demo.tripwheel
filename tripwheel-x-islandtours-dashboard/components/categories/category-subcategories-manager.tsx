...
export function CategorySubcategoriesManager({ parent }: Props) {
  ...
  const { mutate: updateCategory, isPending: isDetaching } = useUpdateCategory();
  ...
  function handleAdd() {
    ...
    createCategory(
      { name: trimmed, slug: toSlug(trimmed), parentCategoryId: parent.id },
      {
        onSuccess: () => {
 ...
 if (e.key ==='Enter') {
            ...
            <p className="text-xs text-muted-foreground">
              Creates a brand-new filter-only sub-category. Existing top-level
              categories are never pulled in here, so their pages stay intact.
            </p>
          </div>
        )}
      </CardContent>
