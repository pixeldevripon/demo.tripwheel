...
> = { DRAFT: 'secondary', PUBLISHED: 'default', ARCHIVED: 'destructive' };
...
export function HubEditView({ id, initialTab }: HubEditViewProps) {
    ...
    const enDiscoverCount =
        contentSections?.filter(
            s => s.locale === 'en' && s.sectionType === 'DISCOVER'
        ).length ?? 0;
    const enLocalTipCount =
        contentSections?.filter(
            s => s.locale === 'en' && s.sectionType === 'LOCAL_TIP'
        ).length ?? 0;
    ...
    const readinessChecks = [
        ...
        {
            label: 'English H1 override filled',
            passed: !!enTranslation?.h1Override?.trim(),
        },
        {
            label: 'English overview filled',
            passed: !!enTranslation?.overview?.trim(),
        },
        {
            label: 'At least 1 English Discover section',
            passed: enDiscoverCount >= 1,
        },
        {
            label: 'At least 1 English Local Tip section',
            passed: enLocalTipCount >= 1,
        },
    ];
            ...
            <div className='space-y-6'>
                ...
                <div className='flex flex-wrap items-center justify-between gap-3'>
                    <div className='flex items-center gap-3'>
                        ...
                        <Badge variant={statusVariant[hub.status]}>
