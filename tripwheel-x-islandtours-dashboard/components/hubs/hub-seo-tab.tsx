      ...
      <CardContent className="pt-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          <Field>
            ...
            />
            <FieldDescription>
              Image shown when this hub is shared on social media (the Open Graph image).
...
// metaTitle / metaDescription live on HubPageContent; saved via a partial
...
function MetaLocalePanel({ hub, locale, isEnglish = false }: MetaLocalePanelProps) {
  const { data: content, isLoading } = useHubPageContent(hub.id, locale);
  const { data: translation } = useHubTranslationByLocale(hub.id, locale);
  ...
  // English falls back to the hub's canonical name/overview; other locales use
  // their translated name/overview when present.
  const localizedName = translation?.name || hub.name;
  const localizedOverview = isEnglish
    ? translation?.overview || hub.overview || ''
    : translation?.overview || '';

  const suggestedTitle = truncate(
    collapse(`${localizedName} Guide`),
    META_TITLE_MAX,
  );
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <Field>
        ...
        <div className="flex items-center justify-between gap-3">
          <FieldDescription>
            Pre-filled from the {isEnglish ? 'hub name' : 'translated name'}. Edit to customize.
          </FieldDescription>
          <button
            type="button"
...
interface HubSeoTabProps {
  hub: HubDetail;
}

export function HubSeoTab({ hub }: HubSeoTabProps) {
  return (
        ...
        <CardContent className="pt-6">
          ...
          <Tabs defaultValue="en">
            ...
            <TabsContent value="en">
              <MetaLocalePanel hub={hub} locale="en" isEnglish />
            </TabsContent>

            {ALL_LOCALES.filter((l) => l !== 'en').map((locale) => (
              <TabsContent key={locale} value={locale}>
                <MetaLocalePanel hub={hub} locale={locale} />
              </TabsContent>
            ))}
          </Tabs>
        </CardContent>
