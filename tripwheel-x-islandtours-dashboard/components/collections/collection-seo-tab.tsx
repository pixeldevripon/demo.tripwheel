...
// ── Social sharing (collection OG image) ─────────────────────────────────────
...
function SocialCard({ collection }: { collection: Collection }) {
  const { mutate: updateCollection, isPending } = useUpdateCollection();

  const { handleSubmit, control, reset } = useForm<SocialValues>({
    resolver: zodResolver(socialSchema),
    defaultValues: { ogImage: collection.ogImage ?? '' },
  });

  useEffect(() => {
    reset({ ogImage: collection.ogImage ?? '' });
  }, [collection, reset]);

  function onSubmit(values: SocialValues) {
    updateCollection(
      { id: collection.id, payload: { ogImage: values.ogImage || null } },
      {
      ...
      <CardContent className="pt-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          <Field>
            ...
            />
            <FieldDescription>
              Image shown when this collection is shared on social media (the Open Graph image).
...
// metaTitle / metaDescription live on CollectionPageContent; saved via a partial
  ...
  return (
    <div className="border border-border bg-muted/30 px-4 py-3 space-y-1">
      ...
      <p className="text-[13px] leading-[1.4] text-success-fg truncate">
        islandtours.com › ... › {slug || 'collection'}
      </p>
      <p className="text-[18px] leading-[1.3] text-[#1a0dab] truncate">
        {title || `${collectionName || 'Collection'} | Island Tours`}
      </p>
...
function MetaLocalePanel({ collection, locale, isEnglish = false }: MetaLocalePanelProps) {
  const { data: content, isLoading } = useCollectionPageContent(collection.id, locale);
  const { data: translation } = useCollectionTranslationByLocale(collection.id, locale);
  ...
  // English falls back to the collection's canonical name; other locales use
  // their translated name/overview when present.
  const localizedName = translation?.name || collection.name;
  const localizedOverview = translation?.overview || '';

  const suggestedTitle = truncate(
    collapse(`${localizedName} | Island Tours`),
    META_TITLE_MAX,
  );
 ...
 {...register('metaTitle')}
        ...
        <div className="flex items-center justify-between gap-3">
          <FieldDescription>
            Pre-filled from the {isEnglish ? 'collection name' : 'translated name'}. Edit to customize.
          </FieldDescription>
          <button
            type="button"
...
export function CollectionSeoTab({ collection }: CollectionSeoTabProps) {
 ...
 <CardContent className="pt-6">
 <div className="text-xs text-muted-foreground bg-muted px-3 py-2 mb-6">
 <p>
 ...
 <MetaLocalePanel collection={collection} locale="en" isEnglish />
 </TabsContent>

 {ALL_LOCALES.filter((l) => l !=='en').map((locale) => (
              <TabsContent key={locale} value={locale}>
                <MetaLocalePanel collection={collection} locale={locale} />
              </TabsContent>
            ))}
          </Tabs>
        </CardContent>
      ...
      <SocialCard collection={collection} />
    </div>
  );
}
