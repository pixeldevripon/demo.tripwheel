...
const schema = z.object({
    ...
    displayStyle: z.enum(
        COLLECTION_DISPLAY_STYLE_VALUES as [string, ...string[]]
    ),
...
export function CollectionForm({
    ...
    const {
        ...
        defaultValues: {
            ...
            cancellationMaxHours:
                fq.cancellationMaxHours != null
                    ? String(fq.cancellationMaxHours)
                    : '',
            pickupAvailable:
                typeof fq.pickupAvailable === 'boolean'
                    ? String(fq.pickupAvailable)
                    : '',
            isLocalsFavourite: fq.isLocalsFavourite === true ? 'true' : '',
            pricingModel:
                typeof fq.pricingModel === 'string' ? fq.pricingModel : '',
        },
    });
    ...
    const { data: allFilterableAttrs } = useAttributes({
        filterableOnly: true,
    });
    ...
    const scopedAttributes = useMemo(
        () =>
            (allFilterableAttrs ?? []).filter(a => {
                ...
                if (
                    !a.appliesToCategories ||
                    a.appliesToCategories.length === 0
                )
                    return true;
    ...
    function buildFilterQuery(values: FormValues): CollectionFilterQuery {
        ...
        if (num(values.minPrice) !== undefined)
            q.minPrice = num(values.minPrice);
        if (num(values.maxPrice) !== undefined)
            q.maxPrice = num(values.maxPrice);
        if (num(values.durationMin) !== undefined)
            q.durationMin = num(values.durationMin);
        if (num(values.durationMax) !== undefined)
            q.durationMax = num(values.durationMax);
        if (num(values.ratingMin) !== undefined)
            q.ratingMin = num(values.ratingMin);
        ...
        if (values.pricingModel)
            q.pricingModel =
                values.pricingModel as CollectionFilterQuery['pricingModel'];
            ...
            update(
                {
                    ...
                    payload: {
                        ...
                        displayStyle:
                            values.displayStyle as CollectionDisplayStyle,
                        ...(isManual
                            ? {}
                            : { filterQuery: buildFilterQuery(values) }),
                    },
                },
                {
            ...
            create(
                {
                    ...
                    collectionType:
                        values.collectionType as Collection['collectionType'],
                    heroImage: values.heroImage || null,
                    sortOrder: values.sortOrder,
                    ...
                    <Field>
                        ...
                        {isEditMode ? (
                            ...
                            <Input
                                ...
                                placeholder='e.g. best-family-trips'
                                aria-invalid={
                                    !!errors.slug || slugClashesCategory
                                }
                                onChange={e => {
                                    setSlugTouched(true);
                        ...
                        {!isEditMode && slugClashesCategory && (
                            <div className='flex items-center gap-1.5 text-xs text-warning-fg'>
                                ...
                                />
                                This slug matches a category slug - the server
                                will reject it (409). Pick a different slug.
                            </div>
                        )}
                        <FieldDescription>
                            {isEditMode
                                ? 'Slug cannot be changed after creation.'
                                : 'Must not match a category slug.'}
                        </FieldDescription>
                        {!isEditMode && (
              ...
              filter resolves + orders tours). MANUAL order is the hand-picked list. */}
                    {collectionType === 'DYNAMIC' && (
                        <Field>
                            ...
                            <FieldDescription>
                                How the filtered tours are ordered on the
                                collection page.
                            </FieldDescription>
                        </Field>
                    )}
                    ...
                    <Field>
                        ...
                        <FieldDescription>
                            {isEditMode
                                ...
                                : 'Manual = ordered tour list; Dynamic = saved filter.'}
                        </FieldDescription>
                    </Field>
                    ...
                    {collectionType === 'MANUAL' ? (
                        isEditMode ? (
                            <div className='flex items-start gap-3 rounded-md border border-border bg-muted/40 p-4'>
                                ...
                                <div className='space-y-2 text-sm'>
                                    <p className='text-muted-foreground'>
                                        Tour selection, ordering, and per-tour
                                        rationale are managed in the Tours tab.
                                    </p>
                                    <Button
                                        type='button'
                                        variant='outline'
                            ...
                            <Field>
                                ...
                                />
                                <FieldDescription>
                                    Selection order is the display order.
                                </FieldDescription>
                            </Field>
                        )
                    ) : (
                        <div className='space-y-6 rounded-md border border-border p-4'>
                            ...
                            <Field>
                                ...
                                <FieldDescription>
                                    Only tours tagged with this activity hub.
                                </FieldDescription>
                            </Field>
                            <div className='grid gap-4 sm:grid-cols-2'>
                                ...
                                <Field>
                                    <Label>
                                        Free-cancellation window (max hrs)
                                    </Label>
                                    <Input
                                        type='number'
                                        min={0}
                                        {...register('cancellationMaxHours')}
                                        placeholder='e.g. 48'
                                    />
                                </Field>
                            </div>
                            ...
                            {scopedAttributes.length > 0 && (
                                <div className='space-y-4 border-t border-border pt-4'>
                                    ...
                                    {!categoryId && (
                                        <FieldDescription>
                                            Showing global attributes. Pick a
                                            category above to also filter by
                                            that category&apos;s attributes.
                                        </FieldDescription>
                                    )}
                                    <div className='grid gap-4 sm:grid-cols-2'>
