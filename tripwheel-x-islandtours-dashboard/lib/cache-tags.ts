...
export const COARSE_CACHE_TAGS = [
    ...
    'homepage',
    // Island Tours' post-booking recommendations, featured at the foot of the
    // thank-you page (Dashboard > Recommendations). Its own tag, not `homepage`:
    // it renders on a different page, and it is the one section whose content
    // decides whether it renders AT ALL - switching a recommendation off, or
    // clearing its photo, title or link, changes WHICH one the card shows or
    // takes the card down entirely. That save has to take effect now rather than
    // after a cacheLife('days').
    //
    // Coarse rather than per-id: the public read serves whichever recommendation
    // wins the promotion order for a surface, so editing row B can change the
    // card row A was filling. A per-id tag could not express that.
    'recommendations',
    ...
    'media-indexing',
    // Per-locale media SEO copy (alt text, title, description), read by
    // `getMediaSeo` to caption og:image, gallery photos and heroes in the
    // visitor's language. Separate from `media-indexing` because that one is a
    // single platform-wide URL list while this is per-asset, per-locale text.
    //
    // Coarse rather than per-id: entity tables store only the image URL, never a
    // media id, so the producer of an alt-text edit cannot know which tours,
    // destinations or hubs render that asset. There is no id it could name.
    'media-seo',
