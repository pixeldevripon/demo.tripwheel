...
export const collectionKeys = {
    ...
    byDestination: (destinationSlug: string) =>
        [...collectionKeys.all, 'admin', destinationSlug] as const,
    detail: (id: string) => [...collectionKeys.all, 'detail', id] as const,
    translations: (id: string) =>
        [...collectionKeys.all, 'translations', id] as const,
    ...
    faqs: (id: string, locale?: Locale) =>
        [...collectionKeys.all, 'faqs', id, locale] as const,
    toursForEdit: (id: string) =>
        [...collectionKeys.all, 'tours-edit', id] as const,
    // Child of detail(id) so invalidating the detail (e.g. after a filterQuery save)
    // also refreshes the resolved-tours preview.
    resolvedTours: (id: string) =>
        [...collectionKeys.detail(id), 'resolved'] as const,
};

/** Pass the `'all'` sentinel to list collections across every island. */
export function useCollectionsByDestination(
    ...
    return useMutation({
        ...
        onSuccess: (_d, vars) => {
            qc.invalidateQueries({
                queryKey: collectionKeys.translations(vars.id),
            });
            qc.invalidateQueries({ queryKey: collectionKeys.detail(vars.id) });
        },
    });
...
export function useUpsertCollectionPageContent() {
    ...
    return useMutation({
        ...
        onSuccess: (_d, vars) => {
            ...
            qc.invalidateQueries({
                queryKey: [...collectionKeys.all, 'page-content', vars.id],
            });
        },
    });
}
...
export function useCreateCollectionFaq() {
    ...
    return useMutation({
        mutationFn: ({
            ...
            id: string;
            payload: CreateCollectionFaqPayload;
...
export function useReplaceCollectionTours() {
    ...
    return useMutation({
        mutationFn: ({
            ...
            id: string;
            payload: ReplaceCollectionToursPayload;
        }) => collectionsApi.replaceTours(id, payload),
        onSuccess: (_d, vars) => {
            qc.invalidateQueries({ queryKey: collectionKeys.detail(vars.id) });
            qc.invalidateQueries({
                queryKey: collectionKeys.toursForEdit(vars.id),
            });
        },
    });
}
